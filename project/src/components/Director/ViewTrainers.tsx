...
// Trainee hover tooltip component
const TraineeTooltip = ({ trainee, position }: { trainee: any, position: { x: number, y: number } }) => (
  <div
...
export default function ViewTrainers() {
  ...
  const [hoveredTrainee, setHoveredTrainee] = useState<{ trainee: any, position: { x: number, y: number } } | null>(null);

  // Use actual trainers from context with trainee counts
  ...
  const handleTraineeHover = (trainee: any, event: React.MouseEvent) => {
    setHoveredTrainee({
      trainee,
  ...
  const exportTraineeProfile = (trainee: any) => {
