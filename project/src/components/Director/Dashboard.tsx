...
export default function DirectorDashboard({ onNavigate }: DirectorDashboardProps) {
  ...
  const generateRecentActivities = () => {
    ...
    // Recent trainee registrations
    const recentTrainees = trainees
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
      .slice(0, 2);
    
    recentTrainees.forEach((trainee, index) => {
      const timeAgo = getTimeAgo(trainee.createdAt);
      activities.push({
